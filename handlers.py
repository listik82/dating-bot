...
async def process_phone_text(message: Message, state: FSMContext):
    ...
    if cleaned.startswith("998") and len(cleaned) == 12 and cleaned[3:].isdigit():
        ...
        await message.answer(
            "❌ Принимаются только узбекские номера.
"
...
async def cb_fake_delete(callback: CallbackQuery, state: FSMContext):
    ...
    conn = sqlite3.connect(db.DB_NAME)
    c = conn.cursor()
    c.execute("DELETE FROM profiles WHERE user_id = ? AND is_fake = 1", (fake_id,))
    c.execute("DELETE FROM photos WHERE user_id = ?", (fake_id,))
    c.execute("DELETE FROM actions WHERE to_user = ?", (fake_id,))
    conn.commit()
    conn.close()
